...
const sliderInit = function () {
  ...
  autoMoveSlide = setInterval(function () {
    slideCounter +=
      slideCounter >= promoSlides.length - 1 ? -(promoSlides.length - 1) : 1;
    moveSlides(slideCounter, windowWidth, promoSliderWrapper);
  }, 3500);
};
...
promoSlider.addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('slider__left')) {
    slideCounter += slideCounter ? -1 : promoSlides.length - 1;
  }
  moveSlides(slideCounter, windowWidth, promoSliderWrapper);
});
...
const wrapperWidth = `${
  imgWidth * slidesSmall.length + 2 + imgGap * slidesSmall.length
}px`;
...
sliderSmall.addEventListener('click', function (e) {
  if (e.target.classList.contains('slider__right')) {
    ...
    sliderSmallWrapper.style.transform = `translateX(${-(
      imgWidth + imgGap
    )}px)`;

    sliderSmallWrapper.insertAdjacentElement(
      'afterbegin',
      sliderSmallWrapper.lastElementChild
    );
  }
  if (e.target.classList.contains('slider__left')) {
    sliderSmallWrapper.insertAdjacentElement('beforeend', sliderFirstEl);
  }
});
